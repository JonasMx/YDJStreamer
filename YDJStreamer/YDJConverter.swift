...
open class YDJConverter: NSObject {
    /**
     YDJConverterCallback is the callback format for start()
     -Parameter: error This will contain one parameter of type Error which is nil if the conversion was successful.
     */
    public typealias YDJConverterCallback = (_ error: Error?) -> Void
    
    /** Formats that this class can write */
    public static let outputFormats = ["wav", "aif", "caf", "m4a"]
    
    /** Formats that this class can read */
    public static let inputFormats = YDJConverter.outputFormats + ["mp3", "mp4", "snd", "au", "sd2", "aiff", "aifc", "aac"]
     ...
     */
    open func start(completionHandler: YDJConverterCallback? = nil) {
        ...
        // verify inputFormat
        guard YDJConverter.inputFormats.contains(inputFormat) else {
    ...
    private func convertAsset(completionHandler: YDJConverterCallback? = nil) {
        guard let inputURL = self.inputURL else {
            completionHandler?(createError(message: "Input file can't be nil."))
            return
        }
        ...
        // verify outputFormat
        guard YDJConverter.outputFormats.contains(outputFormat) else {
        ...
        let queue = DispatchQueue(label: "io.audiokit.YDJConverter.start", qos: .utility)
    ...
    private func convertCompressed(completionHandler: YDJConverterCallback? = nil) {
        guard let inputURL = self.inputURL else {
            completionHandler?(createError(message: "Input file can't be nil."))
            return
        }
    ...
    private func convertToPCM(completionHandler: YDJConverterCallback? = nil) {
        guard let inputURL = self.inputURL else {
            completionHandler?(createError(message: "Input file can't be nil."))
            return
        }
    ...
    private func createError(message: String, code: Int = 1) -> NSError {
        ...
        return NSError(domain: "io.audiokit.YDJConverter.error", code: code, userInfo: userInfo)
    }
}
